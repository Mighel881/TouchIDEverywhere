                    ...
                    CFDictionaryRef userInfo)
{
    if (currentMonitoringField) {
    	...
    	CFNotificationCenterRemoveObserver(CFNotificationCenterGetDarwinNotifyCenter(), (void*)observer, CFSTR("com.shade.touchideverywhere/success"), NULL);
		  CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("com.shade.touchideverywhere/stopMonitoring"), nil, nil, YES); // Should already have been done though
    }
}
                    ...
                    CFDictionaryRef userInfo)
{
    if (currentMonitoringField) {
      ...
      dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((animation.duration * (animation.repeatCount * 2)) * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void){
    	...
    	CFNotificationCenterRemoveObserver(CFNotificationCenterGetDarwinNotifyCenter(), (void*)observer, CFSTR("com.shade.touchideverywhere/failure"), NULL);
	}
}
...
- (void)_endedEditing {
	...
	CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("com.shade.touchideverywhere/stopMonitoring"), nil, nil, YES);
...
- (void)_becomeFirstResponder {
	...
	if (self == associatedUsernameField || self.secureTextEntry) {
		...
		CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), (void*)observer, &touchIdSuccess, CFSTR("com.shade.touchideverywhere/success"), NULL, 0);
		CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), (void*)observer, &touchIdFail, CFSTR("com.shade.touchideverywhere/failure"), NULL, 0);
	  CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("com.shade.touchideverywhere/startMonitoring"), nil, nil, YES);
	}
}
